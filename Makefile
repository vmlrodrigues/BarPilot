...
dmg: check
	...
	@echo "-> Signing the DMG itself (create-dmg leaves it unsigned, so Gatekeeper"
	@echo "   has no signature to assess on the container - only the app inside)..."
	codesign --sign "$(RELEASE_SIGN_IDENTITY)" --timestamp "$(DMG_NAME)"
	...
	@spctl --assess --type open --context context:primary-signature --ignore-cache "$(DMG_NAME)" && echo "   Gatekeeper (dmg): OK" || echo "   WARNING: Gatekeeper rejected the DMG - review signing"
	@spctl --assess --type execute --ignore-cache "$(STAGING)/$(APP)" && echo "   Gatekeeper (app): OK" || echo "   WARNING: Gatekeeper check failed - review signing/entitlements"
	@rm -rf "$(STAGING)"
